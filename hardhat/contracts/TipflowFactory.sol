...
contract TipflowFactory is Ownable {
    struct CreatorInfo {
        string username;
    ...
    /// @dev Function to deploy a new Tipflow contract
    ...
    function deployContract(
        ...
        Tipflow tipflow = new Tipflow(address(this));
        tipflow.transferOwnership(msg.sender);

        CreatorInfo memory creatorInfo = CreatorInfo({
            username: username,
            creatorAddress: msg.sender,
            contractAddress: address(tipflow)
        });
        ...
        emit ContractDeployed(msg.sender, address(tipflow));

        return address(tipflow);
    }
